...
FRAME_SIZE = 129
HEADER = b'\xAA\x55\x01'
ping_packet = HEADER + bytes(FRAME_SIZE - 4)
...
dir = "./passagens/"
file_name = "ping_pong_128_" + time.strftime("%H%M%S")
...
f.write(
    ...
    "HOUR_PC,MIN_PC,SEC_PC,CENT_PC,LATENCIA"
...
while True:
    ...
    # espera 10 segundos por resposta (pong)
    while time.monotonic() - envio < 10:
        
        package = ser.read(FRAME_SIZE)

        if len(package) == FRAME_SIZE:
            latencia = (time.monotonic() - envio) / 2
            ...
            f.write(
                ...
                f"{hora_pc},{minuto_pc},{segundo_pc},{cent_pc},{latencia},"
